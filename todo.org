...
*** (partial f 1 2) where f is a LVAL_REF (so local, closure or arg), is a fn (or not)
...
*** (let [p partial], so when the partial fn is a LVAL_REF, we need to be able to datafy the partial fn
    (p f 1 2))
