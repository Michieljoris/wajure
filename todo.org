...
Branching: and or when when-not when-let when-first if-not if-let cond condp cond-> cond->>
Looping (see also Sequences): for doseq dotimes while
Working with vars (see also Vars and Environment): ns declare defmethod defmulti defn- defonce
