...
** when macros return a function (when called from within another fn), do they deduplicated?
Since we're compiling the the expanded macro as if it's code in place do we know that the wasm fn might already exist?
