...
*** When referring to LVAL_REF/wval only set to local when it was a fn call.
So LOCAL and PARAM (don't set to local) vs closure (set to local)
...
*** And what about (partial partial 1 2) or where either partial is an lval_ref or result of fn call!!!
*** in native partial fn:
   TODO: make sure first arg is a wval_fn!!!! Or something that can be turned
...
** multimethods
** try/throw/catch
** implement: quasiquote, doseq, keywords, map, reduce, and, or, xor, doseq, destructering, meta data, atoms
