ifeq ($(PLATFORM),wasm)
src = src/platform.c src/malloc.c src/refcount.c src/mempool.c src/printf.c src/lispy_mempool.c  src/lib.c  src/io.c src/print.c src/list.c src/cell.c  src/lval.c src/util_fns.c src/iter.c src/math_fns.c src/list_fns.c src/runtime.c
...
ifeq ($(PLATFORM),wasm)
	...
	CFLAGS = $(X86FLAGS)
	LDFLAGS = $(X86LDFLAGS)
	COMPILE = out/lispy -c clj/compile.lispy;  ~/bin/node nodejs.js
