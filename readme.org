...
- Another feature: it's possible to request an arbitrarily sized object but only
  to a maximum size. Under the cover however a selection is made from an memory
  pool with appropriately sized objects. Waste will be at most the space of the
  object itself, but on average a third of the object (eg. any size between 128
  and 256 will be allocated 256 bytes, which will be at most 127 bytes waste and
  at minimum 0, so with even distribution of sizes requested will mean on
  average 63-64 bytes waste for this memory pool). The smaller the objects that
  are requested the less the wastage actually matters. This is suitable for
...
- Memory only ever grows. Once a slot is released it's available again for
  ...
  useable for programs that might peak in requiring memory but most of the time
  need much less, so for longer running programs with unsteady and unpredictable
  ...
  that don't need to hang on to state from job to job and can rest their memory
  pool.
